...
def append_rows(path, rows):
    ...
    for row in rows:
        ...
        if unknown or missing:
            raise RuntimeError(f"schema mismatch for {row.get(fieldnames[0])}: unknown={unknown} missing={missing}")
        if row[fieldnames[0]] in ids:
            # Idempotent retry: require exact semantic identity of the already materialized record.
...
# Relational read-back before rebuilding derived artifacts.
products = read_rows(PRODUCTS)
dists = read_rows(DISTS)
pids = {r["product_id"] for r in products}
if len(pids) != len(products):
    raise RuntimeError("duplicate product_id after append")
if len({r["distribution_id"] for r in dists}) != len(dists):
    raise RuntimeError("duplicate distribution_id after append")
if any(r["product_id"] not in pids for r in dists):
    raise RuntimeError("distribution FK violation")
for pid in ("DP000757", "DP000758"):
    if sum(r["product_id"] == pid for r in products) != 1:
        raise RuntimeError(f"product read-back failed: {pid}")
    if sum(r["product_id"] == pid for r in dists) != 1:
        raise RuntimeError(f"distribution read-back failed: {pid}")
