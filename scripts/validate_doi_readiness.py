...
if readiness.get("catalog_state") != "release_candidate":
    fail("catalog_state deve ser release_candidate na branch candidata")
...
if 'version: "1.0.0"' not in citation:
    fail("CITATION.cff da candidata deve declarar version 1.0.0")
for required in (
    "Vitrine Ciência",
...
if "ainda sem tag Git imutável, GitHub Release ou DOI" not in project_state:
    fail("PROJECT_STATE.md não preserva o estado não publicado da candidata")
...
print(
    "OK: prontidão DOI da candidata alinhada ao modelo atual — "
